...
class DbusShelly3emService:
    ...
    def _getShellySerial(self):
        # meter_data = self._getShellyEmData()
        # if not meter_data['mac']:
        # raise ValueError("Response does not contain 'mac' attribute")

        serial = 'Shelly3ProEM'  # meter_data['mac']
        return serial
    ...
    def _getShellyEmStatusUrl(self):
        config = self._getConfig()
        accessType = config['DEFAULT']['AccessType']

        if accessType == 'OnPremise':
            URL = "http://%s:%s@%s/rpc/EM.GetStatus?id=0" % (
            config['ONPREMISE']['Username'], config['ONPREMISE']['Password'], config['ONPREMISE']['Host'])
            URL = URL.replace(":@", "")
        else:
    ...
    def _getShellyEmDataStatusUrl(self):
        config = self._getConfig()
        accessType = config['DEFAULT']['AccessType']

        if accessType == 'OnPremise':
            URL_EM_DATA = "http://%s:%s@%s/rpc/EMDATA.GetStatus?id=0" % (
            config['ONPREMISE']['Username'], config['ONPREMISE']['Password'], config['ONPREMISE']['Host'])
            URL_EM_DATA = URL_EM_DATA.replace(":@", "")
        else:
            raise ValueError("AccessType %s is not supported" % (config['DEFAULT']['AccessType']))

        return URL_EM_DATA

    def _getShellyEmData(self):
        URL = self._getShellyEmStatusUrl()
    ...
    def _getShellyEMDataData(self):
        URL = self._getShellyEmDataStatusUrl()
        meter_r = requests.get(url=URL, timeout=5)
        ...
        meter_em_data = meter_r.json()

        # check for Json
        if not meter_em_data:
            raise ValueError("Converting response to JSON failed")

        return meter_em_data
    ...
    def _update(self):
        try:
            ...
            meter_data = self._getShellyEmData()
            meter_em_data = self._getShellyEMDataData()
            ...
            self._dbusservice['/Ac/L1/Energy/Forward'] = (meter_em_data['a_total_act_energy'] / 1000)
            self._dbusservice['/Ac/L2/Energy/Forward'] = (meter_em_data['b_total_act_energy'] / 1000)
            self._dbusservice['/Ac/L3/Energy/Forward'] = (meter_em_data['c_total_act_energy'] / 1000)
            self._dbusservice['/Ac/L1/Energy/Reverse'] = (meter_em_data['a_total_act_ret_energy'] / 1000)
            self._dbusservice['/Ac/L2/Energy/Reverse'] = (meter_em_data['b_total_act_ret_energy'] / 1000)
            self._dbusservice['/Ac/L3/Energy/Reverse'] = (meter_em_data['c_total_act_ret_energy'] / 1000)
            ...
            logging.critical(
                'Error getting data from Shelly - check network or Shelly status. Setting power values to 0')
